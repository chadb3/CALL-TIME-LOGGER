...
class callTimeDB:
	...
	def SELECT_LAST_CALL_ID(this):
		sqlConnection=sqlite3.connect(this.DB)
		sqlCursor=sqlConnection.execute("SELECT CALL_NUM,CALL_TIME_END,CALL_DATE_END FROM CALL ORDER BY CALL_NUM DESC LIMIT 1")
		try:
			#print("DEBUG: SELECT LAST CALL ID HIT")
